...
I2D* mser(I2D* I, int in_delta)
{
   ...
   int IN_DELTA = 1;   
    ...
    ndims = 2;
    dims = pvPortMalloc(sizeof(iArray) + sizeof(int)*ndims);
    I_pt = I;
    ...
    /* allocate stuff */
    subs_pt = pvPortMalloc(sizeof(iArray) + sizeof(int)*ndims);
    nsubs_pt = pvPortMalloc(sizeof(iArray) + sizeof(int)*ndims);

    strides_pt = pvPortMalloc(sizeof(uiArray)+sizeof(unsigned int)*ndims);
    visited_pt = pvPortMalloc(sizeof(uiArray) + sizeof(unsigned int)*nel);
    joins_pt = pvPortMalloc(sizeof(uiArray) + sizeof(unsigned int)*nel);

    regions_pt = (region_t*)pvPortMalloc(sizeof(region_t)*nel);
    regions_pt_size = nel;

    pairs_pt = (pair_t*)pvPortMalloc(sizeof(pair_t)*nel);
    pairs_pt_size = nel;

    forest_pt = (node_t*)pvPortMalloc(sizeof(node_t)*nel);
    forest_pt_size = nel;
    ...
    * -------------------------------------------------------------- */
    ...
    if( big_cleanup || small_cleanup || bad_cleanup || dup_cleanup ) 
    {
        ...
        if(0) 
        {
            FF_PRINTF("  Bad regions:        %d\n", nbad   ) ;
            FF_PRINTF("  Small regions:      %d\n", nsmall ) ;
            FF_PRINTF("  Big regions:        %d\n", nbig   ) ;
            FF_PRINTF("  Duplicated regions: %d\n", ndup   ) ;
        }
    }

/*    FF_PRINTF("Cleaned-up regions: %d (%.1f%%)\n", 
...
*/
    ...
    * -------------------------------------------------------------- */
    ...
    if (nout >= 1) 
    {
        ...
        /* allocate space */
        acc_pt = pvPortMalloc(sizeof(ulliArray) + sizeof(acc_t)*nel) ;
        ell_pt = pvPortMalloc(sizeof(ulliArray) + sizeof(acc_t)*gdl*nmer) ; 
...
//                *pt++ = regions_pt[i].index + 1 ;
    ...
    vPortFree( forest_pt  ) ;
    vPortFree( pairs_pt   ) ;
    vPortFree( regions_pt ) ;
    vPortFree( visited_pt ) ;
    vPortFree( strides_pt ) ;
    vPortFree( nsubs_pt   ) ;
    vPortFree( subs_pt    ) ;
    vPortFree( joins_pt    ) ;
