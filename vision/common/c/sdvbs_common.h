...
static inline void ff_printf(const char *pcFormat, ...) {
    ...
    while (*pcFormat != '\0') {
        if (*pcFormat == '%') {
            ...
            switch (*pcFormat) {
                case 'd':
                    // Append integer argument to the buffer
                    dst += snprintf(dst, sizeof(buffer) - (dst - buffer), "%d", va_arg(args, int));
                    break;
                case 's': {
                    // Append string argument to the buffer
                    const char *str = va_arg(args, const char *);
                    if (str != NULL) {
                        dst += snprintf(dst, sizeof(buffer) - (dst - buffer), "%s", str);
                    }
                    break;
                }
            ...
            // Copy non-format-specifier characters to the buffer
            *dst++ = *pcFormat;
        }
